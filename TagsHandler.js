...
async function fetchTags() {
    ...
    let data3 = await response3.json()
        .then(function (data3) {
            handle_data(data3.data, amenity_array);
            all_results_array = all_results_array.concat(amenity_array);
        });
    ...
    let data4 = await response4.json()
        .then(function (data4) {
            handle_data(data4.data, amenity_array);
            all_results_array = all_results_array.concat(amenity_array);
        });
    ...
    let data5 = await response5.json()
        .then(function (data5) {
            handle_data(data5.data, amenity_array);
            all_results_array = all_results_array.concat(amenity_array);
        });

    //autocomplete(document.getElementById("myInput"), all_results_array);
}
fetchTags();
